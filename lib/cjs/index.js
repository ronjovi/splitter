...
function getInnerSize(direction, element) {
    ...
    if (!computedStyle)
        return;
    let size = direction === exports.SplitDirection.Horizontal ? element.clientWidth : element.clientHeight;
    if (size === 0)
        return;
...
const Gutter = React__default["default"].forwardRef(({ className, theme, draggerClassName, direction = exports.SplitDirection.Vertical, onDragging, }, ref) => {
    ...
    return (React__default["default"].createElement("div", { className: containerClass, ref: ref, dir: direction, onMouseDown: onDragging, onTouchStart: isTouchDevice ? onDragging : undefined },
        React__default["default"].createElement("div", { className: draggerClass })));
});
...
function reducer(state, action) {
    switch (action.type) {
        ...
        case ActionType.CreatePairs: {
            ...
            children.forEach((_, idx) => {
                if (idx > 0) {
                    ...
                    const start = direction === exports.SplitDirection.Horizontal
                        ? a.getBoundingClientRect().left
                        : a.getBoundingClientRect().top;
                    const end = direction === exports.SplitDirection.Horizontal
                        ? b.getBoundingClientRect().right
                        : b.getBoundingClientRect().bottom;
                    const size = direction === exports.SplitDirection.Horizontal
                    ...
                    const gutterSize = direction === exports.SplitDirection.Horizontal
        ...
        case ActionType.CalculateSizes: {
            ...
            const gutterSize = pair.gutter[direction === exports.SplitDirection.Horizontal ? 'clientWidth' : 'clientHeight'];
...
exports.GutterTheme = void 0;
(function (GutterTheme) {
    GutterTheme["Light"] = "Light";
    GutterTheme["Dark"] = "Dark";
})(exports.GutterTheme || (exports.GutterTheme = {}));
...
function Split({ direction = exports.SplitDirection.Horizontal, minWidths = [], minHeights = [], initialSizes, gutterTheme = exports.GutterTheme.Dark, gutterClassName, draggerClassName, children: reactChildren, onResizeStarted, onResizeFinished, classes = [], }) {
    ...
    const setIsReadyToCompute = React__default["default"].useCallback((isReady) => {
        dispatch({
            type: ActionType.SetIsReadyToCompute,
            payload: { isReady },
        });
    }, []);
    const startDragging = React__default["default"].useCallback((direction, gutterIdx) => {
        dispatch({
    ...
    const stopDragging = React__default["default"].useCallback(() => {
        dispatch({
        ...
        for (let idx = 0; idx < state.pairs.length; idx++) {
            ...
            const aSize = pair.a.getBoundingClientRect()[direction === exports.SplitDirection.Horizontal ? 'width' : 'height'];
            ...
            if (isLast) {
                const bSize = pair.b.getBoundingClientRect()[direction === exports.SplitDirection.Horizontal ? 'width' : 'height'];
    ...
    const calculateSizes = React__default["default"].useCallback((direction, gutterIdx) => {
        dispatch({
            type: ActionType.CalculateSizes,
            payload: { direction, gutterIdx },
        });
    }, []);
    const createPairs = React__default["default"].useCallback((direction, children, gutters) => {
        dispatch({
    ...
    const setInitialSizes = React__default["default"].useCallback((direction, children, gutters, initialSizes) => {
        // All children must have common parent.
        const parent = children[0].parentNode;
        if (!parent)
        ...
        children.forEach((c, idx) => {
            ...
            if (children.length > 1) {
                ...
                gutterSize = gutter.getBoundingClientRect()[direction === exports.SplitDirection.Horizontal ? 'width' : 'height'];
    ...
    // |                     |||                     |||                     |
    // -----------------------------------------------------------------------
    const adjustSize = React__default["default"].useCallback((direction, offset) => {
        if (state.draggingIdx === undefined)
    ...
    const drag = React__default["default"].useCallback((e, direction, minSizes) => {
        if (!state.isDragging)
            return;
        if (state.draggingIdx === undefined)
    ...
    const onMove = (e) => {
        ...
        drag(e, direction, direction === exports.SplitDirection.Horizontal ? minWidths : minHeights);
    };
    ...
    React.useEffect(function watchParentSize() {
        ...
        const observer = new ResizeObserver(() => {
            ...
            const size = direction === exports.SplitDirection.Horizontal ? el.clientWidth : el.clientHeight;
    ...
    React.useEffect(function initialSetup() {
        if (!state.isReady)
            return;
    ...
    return (React__default["default"].createElement("div", { className: '__dbk__container ' + `${direction}`, ref: containerRef }, state.isReady && children.map((c, idx) => (React__default["default"].createElement(React__default["default"].Fragment, { key: idx },
        React__default["default"].createElement("div", { ref: el => addRef(childRefs, el), className: '__dbk__child-wrapper ' + (idx < classes.length ? classes[idx] : '') }, c),
        idx < children.length - 1 &&
            React__default["default"].createElement(Gutter, { ref: el => addRef(gutterRefs, el), className: gutterClassName, theme: gutterTheme, draggerClassName: draggerClassName, direction: direction, onDragging: () => handleStartDragging(idx) }))))));
}
